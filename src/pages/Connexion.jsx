...
const Connexion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (response.ok) {
      ...
      navigate("/Accueil"); // Redirection vers la page /Accueil
      window.location.reload(); // Recharger la page /Accueil
      ...
      <div className="flex items-center justify-center min-h-screen mx-auto w-auto lg:w-1/2 xl:w-auto"> 
        <div className="border-[#0000FF] border-2 flex w-auto p-4 lg:p-0 xl:w-1/2 rounded-lg doto">
          ...
          <form className="flex flex-col items-center w-full px-2 md:px-4 xl:w-1/2 py-3 xl:py-6" onSubmit={handleSubmit}>
            <h1 className="text-[#0000FF] text-2xl lg:text-3xl font-bold pb-6">
              Connectez-vous
            </h1>
            {/* Champ Email */}
            <input
